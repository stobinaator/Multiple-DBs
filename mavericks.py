...
class Mavericks(db.Model):
    ...
    def get_player_by_first_name(first):
        i = 0
        p_dict = {}
        p_list = Mavericks.query.filter_by(first_name=first).all()
        for p in p_list:
            p_dict[i] = Mavericks.json(p)
            i += 1
    ...
    def get_player_by_last_name(last):
        i = 0
        p_dict = {}
        p_list = Mavericks.query.filter_by(last_name=last).all()
        for p in p_list:
            p_dict[i] = Mavericks.json(p)
            i += 1
    ...
    def get_player_by_pos(pos):
        i = 0
        p_dict = {}
        p_list = Mavericks.query.filter_by(position=pos).all()
        for p in p_list:
            p_dict[i] = Mavericks.json(p)
            i += 1
    ...
    def get_player_by_accomp(accomp):
        i = 0
        p_dict = {}
        p_list = Mavericks.get_all_players()
        final_list = []
        for p in p_list:
            p_dict[i] = p
            i+=1
